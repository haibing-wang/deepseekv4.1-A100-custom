...
run() {  # $1 = batch size
  docker run --rm --gpus "device=$GPU" --ipc=host \
    ...
    --entrypoint vllm $IMG bench latency \
      ...
      --max-model-len 4096 --gpu-memory-utilization 0.60 --max-num-seqs 64 \
      --input-len 512 --output-len 128 --batch-size $1 \
      --num-iters-warmup 2 --num-iters 5 \
      --output-json /results/vllm-w8a8-latency-bs$1.json 2>&1 | grep -v -E "^INFO|^WARNING|^\s*$" | tail -n 12
}
