...
TILED = os.environ.get("DSV41_FP8_TILED", "1") == "1"  # [N/16][K/64][16 rows][64 B]: a warp's loads become one 512-byte range


def tile(w8: torch.Tensor) -> torch.Tensor:
    N, K = w8.shape
    return w8.view(N // 16, 16, K // 64, 64).permute(0, 2, 1, 3).reshape(N, K).contiguous()


def untile(w8: torch.Tensor) -> torch.Tensor:
    N, K = w8.shape
...
class W8:
    ...
    def __init__(self, w8: torch.Tensor, s8: torch.Tensor, permuted: bool = False, tiled: bool | None = None):
        ...
        s8: E8M0 [ceil(N/32), K/32]. The stored bytes are k-permuted (PERM_K) and, when the shape allows, tiled."""
        ...
        if permuted:
            ...
            self.tiled = TILED and N % 16 == 0 and K % 64 == 0
            self.w8 = permute_k(w8)
            if self.tiled:
                self.w8 = tile(self.w8)
    ...
    @staticmethod
    def cat(ws: list) -> "W8":
        """Concatenate along N (row counts must be multiples of 32; tiles of 16 rows stay intact)."""
        assert all(w.shape[0] % 32 == 0 for w in ws) and len({w.tiled for w in ws}) == 1
        return W8(torch.cat([w.w8 for w in ws], dim=0), torch.cat([w.s8 for w in ws], dim=0), permuted=True, tiled=ws[0].tiled)
