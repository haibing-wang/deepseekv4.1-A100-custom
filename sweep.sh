#!/bin/bash
# Full sweep: llama-bench + batched decode + perplexity for each variant.
# All variants share Q8_0 attention; only the MLP (+lm_head) type differs.
# GPU 4 (idle, 38 GiB free) for the 4-bit/8-bit variants; the F16-MLP variant (~46 GiB)
# needs two GPUs (4,5) and waits until the vLLM run on GPU 5 has finished.
set -u
...
run_variant() {  # $1 = gpu list, $2 = variant name
  ...
  CUDA_VISIBLE_DEVICES=$1 $BIN/llama-batched-bench -m $M -ngl 999 -fa 1 -c 16384 -npp 512 -ntg 128 -npl 1,8,32 \
